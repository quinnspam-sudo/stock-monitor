# COMMITTEE DATA PAYLOAD — HOOD (Standard)
Generated: 2026-07-09 13:03 local | Source: stock-monitor local daemon
Instructions: Paste this payload into the Investment Committee session (Phase 1-4
protocol already loaded). Run the 9-member committee, score all 11 categories with
justifications, and output the matching Template.

## Company profile
- Sector: Financial Services | Industry: Capital Markets
- Next earnings: in 20 day(s)

## Trigger(s) breached
- Timing score Δ +15 (threshold ±15)

## Prior ledger state
- Previous Overall Score: 71/110
- Previous Timing Score: 80/100
- Previous Confidence: 65%
- Previous evaluation date: 2026-07-08 17:08

## Local proxy scores (deterministic noise gate — NOT the committee verdict)
- Overall (proxy): 73/110 → local rating band: Watch
- Timing: 95/100
- Data-completeness confidence (proxy): 65%
- Category proxies (2/11 are ESTIMATES from missing data — weigh accordingly):
  - Valuation: 4/10
  - Cash Generation: 5/10  [ESTIMATE — source data missing, do not trust]
  - Growth: 7/10
  - Revenue Visibility: 5/10  [ESTIMATE — source data missing, do not trust]
  - Margin Quality: 10/10
  - Balance Sheet: 5/10
  - Capital Allocation: 6/10
  - Macro Exposure: 3/10
  - Competitive Moat: 10/10
  - Analyst Sentiment: 8/10
  - Technical Position: 10/10

## Evidence-backed factor screen (mechanical; committee should weigh explicitly)
  - Magic Formula rank: #60 of 90 in universe (earnings yield 2.2%, return on capital 27.2%)
  - Piotroski F-Score: 4/9 (1 tests gapped) (8-9 strong, 0-2 weak)
  - PEG (GARP): 3.07
  - Momentum: 6-mo -1.6%, 12-mo +16.6%
  - Quality gates (Buffett-style): ROE >= 15%: PASS; Debt/Equity < 0.5: FAIL; Gross margin > 40%: PASS; CFO > Net Income: FAIL
  - Graham/Buffett intrinsic value: Graham Number $21.84, margin of safety -427.1%
  - CANSLIM (O'Neil): 2/7 criteria met — C: N; A: N; N: N; S: N; L: N; I: Y; M: Y
  - Dividend growth quality (if applicable): not a dividend payer
  - Fama-French 5-factor tilts: Size: Large, Value tilt: Growth (descriptive only, not scored — see interpretation guide), Profitability: +83.3%, Asset growth: +45.6%
  - Quality Minus Junk (AQR-style): 17/100
  - Insider transactions (90d, cluster-buy check): 3 buy txns / 16 sell txns, 1 distinct buyers
  - Analyst estimate revision momentum: -8.3% (current vs 90 days ago)
  - 52-week high breakout (O'Neil/Darvas): -25.2% from 52-wk high
  - Interpretation guide: Magic Formula = Greenblatt cheapness+quality rank within
    the monitored universe; F-Score 8-9 strong / 0-2 weak (Piotroski); PEG <= 1
    attractive per Lynch GARP (ignore for cyclicals); 6-12 mo momentum is the
    Carhart UMD factor; quality gates are Buffett-style quantitative tells.
    Graham Number/margin of safety is asset-and-earnings based (1934-era value
    investing) — expect it to look deeply "overvalued" for asset-light,
    high-multiple growth/tech names; that's the methodology working as
    designed, not a flaw, and should be weighted down for this watchlist's
    growth names accordingly. CANSLIM (O'Neil) is a 7-criterion growth+
    breakout+market-regime checklist, not a continuous score — read the
    per-criterion breakdown, not just the count. Dividend quality only
    applies to actual payers (GAPPED for non-payers, not penalized). Fama-
    French Size/Value tilts are descriptive only and deliberately excluded
    from the conviction blend (a small-cap/cheap-book-to-market tilt would
    bias against this watchlist's mega-cap growth names) — Profitability/
    Investment legs are folded into Quality Minus Junk (AQR-style) instead.
    Insider signal only means something as a cluster (3+ distinct buyers,
    zero sales in 90 days per Lakonishok & Lee) — a single insider trade is
    noise. Analyst revision momentum compares current-quarter/next-year EPS
    estimates to 90 days ago; the 52-week breakout check is O'Neil/Darvas-
    style and distinct from the 3-month-high check in the momentum score above.

## Raw data fields (yfinance)
  - forwardPE: 38.85819
  - trailingPE: 56.151222
  - enterpriseToRevenue: 21.015
  - pegRatio: 3.0747
  - operatingCashflow: 3033999872
  - totalRevenue: 4613000192
  - revenueGrowth: 0.151
  - earningsGrowth: 0.027
  - grossMargins: 0.92239
  - operatingMargins: 0.38519
  - profitMargins: 0.41123
  - ebitdaMargins: 0.0
  - debtToEquity: 140.483
  - currentRatio: 1.114
  - totalCash: 19270000640
  - totalDebt: 13610000384
  - returnOnEquity: 0.21461001
  - returnOnAssets: 0.05187
  - beta: 2.336
  - recommendationMean: 1.71429
  - targetMeanPrice: 110.51538
  - currentPrice: 115.11
  - heldPercentInstitutions: 0.73449
  - numberOfAnalystOpinions: 26

## Recent news headlines
  - Trader turns $800 into $1 million (2026-07-09)
  - 3 Crypto to Watch on Robinhood Chain as Meme Coin Frenzy Takes Off (2026-07-09)
  - Brokers Suggest Investing in Robinhood Markets (HOOD): Read This Before Placing a Bet (2026-07-09)
  - High Yields and Huge Risk From a Bitcoin Giant’s Preferred Shares (2026-07-09)
  - Robinhood Chain DEX Volume Hits Record High Amid Cash Cat Frenzy (2026-07-09)

## Recent insider transactions
  - TENEV VLADIMIR — Sale at price 112.22 - 118.14 per share. (2026-07-06)
  - GALLAGHER DANIEL MARTIN JR — Sale at price 112.06 - 118.45 per share. (2026-07-06)
  - TENEV VLADIMIR —  (2026-07-06)
  - QUIRK STEVEN M — Sale at price 119.96 per share. (2026-07-02)
  - TRESEDER OLUWADARA JOHNSON — Sale at price 115.00 - 120.00 per share. (2026-07-02)

## Gapped data sources (penalize Confidence Score accordingly)
  - Congressional trades: Data Status: GAPPED
  - Options skew / flow: Data Status: GAPPED
  - Institutional flow (13F deltas): Data Status: GAPPED
  - SEC filing stream: Data Status: GAPPED
  - Earnings call transcripts: Data Status: GAPPED
  - Credit ratings: Data Status: GAPPED
  - enterpriseToEbitda: Data Status: GAPPED
  - freeCashflow: Data Status: GAPPED
