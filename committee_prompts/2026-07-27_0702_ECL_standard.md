# COMMITTEE DATA PAYLOAD — ECL (Standard)
Generated: 2026-07-27 07:02 local | Source: stock-monitor local daemon
Instructions: Paste this payload into the Investment Committee session (Phase 1-4
protocol already loaded). Run the 9-member committee, score all 11 categories with
justifications, and output the matching Template.

## Company profile
- Sector: Basic Materials | Industry: Specialty Chemicals
- Next earnings: in 1 day(s)

## Trigger(s) breached
- Earnings imminent: 1 day(s) out — pre-earnings committee review

## Prior ledger state
- Previous Overall Score: 65/110
- Previous Timing Score: 35/100
- Previous Confidence: 70%
- Previous evaluation date: 2026-07-24 13:02

## Local proxy scores (deterministic noise gate — NOT the committee verdict)
- Overall (proxy): 65/110 → local rating band: Watch
- Timing: 36/100
- Data-completeness confidence (proxy): 70%
- Category proxies (1/11 are ESTIMATES from missing data — weigh accordingly):
  - Valuation: 6/10
  - Cash Generation: 3/10
  - Growth: 6/10
  - Revenue Visibility: 5/10  [ESTIMATE — source data missing, do not trust]
  - Margin Quality: 6/10
  - Balance Sheet: 7/10
  - Capital Allocation: 6/10
  - Macro Exposure: 9/10
  - Competitive Moat: 5/10
  - Analyst Sentiment: 8/10
  - Technical Position: 4/10

## Earnings gate (actionable-BUY policy: earnings within 10 business days AND
## strict positivity pass — score >=70, >=3 signals, beat streak >=3/4. A Buy
## rating without this gate is thesis-only, NOT actionable; committee should
## confirm or veto the positivity call using transcript/guidance judgment)
  - Earnings in business days: 1 (buy window: ≤10)
  - Earnings Positivity Score: 44/100 (strict pass bar 70, beat streak 1/4 last quarters)
    - Analyst EPS revision momentum: 32
    - Earnings surprise history: 33
    - Pre-earnings relative strength vs SPY: 16
    - Recommendation upgrades vs downgrades (90d): 90
    - Short interest level & trend: 81
  - GATE VERDICT: NOT ACTIONABLE — positivity 44/100 below strict bar 70; beat streak 1/4 quarters (strict bar: ≥3/4)

## Supermajority consensus vote (last gate before a BUY alert)
  - Verdict: FAIL — 1/7 systems bullish (supermajority 65%, min 6 computable)
    - momentum_timing: ✘
    - factor_tier_high: ✘
    - piotroski: ✘
    - canslim: ✘
    - estimate_revisions: GAPPED
    - insider_signal: GAPPED
    - momentum_6m: GAPPED
    - breakout_52wk: ✘
    - positivity_margin: ✘
    - quality_gates: ✔
  - HARD VETO: hostile market regime: SPY>!50d/200d, VIX 18.2 (max 28.0), term 0.908
  - Market regime: SPY>!50d/200d, VIX 18.2 (max 28.0), term 0.908 — HOSTILE

## Evidence-backed factor screen (mechanical; committee should weigh explicitly)
  - Magic Formula rank: #40 of 102 in universe (earnings yield 3.4%, return on capital 52.2%)
  - Piotroski F-Score: 4/9 (2 tests gapped) (8-9 strong, 0-2 weak)
  - PEG (GARP): 2.56
  - Momentum: 6-mo GAPPED, 12-mo GAPPED
  - Quality gates (Buffett-style): ROE >= 15%: PASS; Debt/Equity < 0.5: FAIL; Gross margin > 40%: PASS; CFO > Net Income: GAPPED
  - Graham/Buffett intrinsic value: Graham Number $76.88, margin of safety -251.6%
  - CANSLIM (O'Neil): 1/7 criteria met (2 gapped) — C: N; A: N; N: N; S: ?; L: ?; I: Y; M: N
  - Dividend growth quality (if applicable): yield 1.0%, payout 37%, quality score 80/100
  - Fama-French 5-factor tilts: Size: Large, Value tilt: Growth (descriptive only, not scored — see interpretation guide), Profitability: +44.5%, Asset growth: +10.3%
  - Quality Minus Junk (AQR-style): 55/100
  - Insider transactions (90d, cluster-buy check): GAPPED
  - Analyst estimate revision momentum: GAPPED
  - 52-week high breakout (O'Neil/Darvas): -12.6% from 52-wk high
  - Interpretation guide: Magic Formula = Greenblatt cheapness+quality rank within
    the monitored universe; F-Score 8-9 strong / 0-2 weak (Piotroski); PEG <= 1
    attractive per Lynch GARP (ignore for cyclicals); 6-12 mo momentum is the
    Carhart UMD factor; quality gates are Buffett-style quantitative tells.
    Graham Number/margin of safety is asset-and-earnings based (1934-era value
    investing) — expect it to look deeply "overvalued" for asset-light,
    high-multiple growth/tech names; that's the methodology working as
    designed, not a flaw, and should be weighted down for this watchlist's
    growth names accordingly. CANSLIM (O'Neil) is a 7-criterion growth+
    breakout+market-regime checklist, not a continuous score — read the
    per-criterion breakdown, not just the count. Dividend quality only
    applies to actual payers (GAPPED for non-payers, not penalized). Fama-
    French Size/Value tilts are descriptive only and deliberately excluded
    from the conviction blend (a small-cap/cheap-book-to-market tilt would
    bias against this watchlist's mega-cap growth names) — Profitability/
    Investment legs are folded into Quality Minus Junk (AQR-style) instead.
    Insider signal only means something as a cluster (3+ distinct buyers,
    zero sales in 90 days per Lakonishok & Lee) — a single insider trade is
    noise. Analyst revision momentum compares current-quarter/next-year EPS
    estimates to 90 days ago; the 52-week breakout check is O'Neil/Darvas-
    style and distinct from the 3-month-high check in the momentum score above.

## Raw data fields (yfinance)
  - forwardPE: 28.884762
  - trailingPE: 36.575104
  - enterpriseToEbitda: 20.889
  - enterpriseToRevenue: 5.131
  - pegRatio: 2.5578
  - freeCashflow: 1586424960
  - operatingCashflow: 3029100032
  - totalRevenue: 16452299776
  - revenueGrowth: 0.1
  - earningsGrowth: 0.078
  - grossMargins: 0.44374
  - operatingMargins: 0.16933
  - profitMargins: 0.12798999
  - ebitdaMargins: 0.24561001
  - debtToEquity: 92.369
  - currentRatio: 0.991
  - totalCash: 519800000
  - totalDebt: 9266499584
  - returnOnEquity: 0.22427
  - returnOnAssets: 0.08009
  - beta: 0.895
  - recommendationMean: 1.65385
  - targetMeanPrice: 319.42856
  - currentPrice: 270.29
  - heldPercentInstitutions: 0.93566
  - numberOfAnalystOpinions: 21

## Recent news headlines
  - Ecolab (ECL) Could Be 17% Undervalued Before June Earnings (2026-07-24)
  - Ecolab Gears Up to Report Q2 Results: How to Play the Stock? (2026-07-24)
  - How A Cut To Ecolab’s (ECL) EPS Estimates Has Changed Its Investment Story (2026-07-24)
  - Ahead of Ecolab (ECL) Q2 Earnings: Get Ready With Wall Street Estimates for Key Metrics (2026-07-23)
  - Chemicals Companies Likely to Report In-Line Quarterly Results, Deutsche Bank Says (2026-07-22)

## Recent insider transactions
  - GROSS MARION K — Stock Award(Grant) at price 0.00 per share. (2026-06-30)
  - DOUKERIS MICHEL — Stock Award(Grant) at price 0.00 per share. (2026-06-30)
  - NOWELL LIONEL L III — Stock Award(Grant) at price 0.00 per share. (2026-06-30)
  - ALTHOFF JUDSON — Stock Award(Grant) at price 0.00 per share. (2026-06-30)
  - MACLENNAN DAVID W — Stock Award(Grant) at price 0.00 - 278.46 per share. (2026-06-30)

## Depth enrichment (short interest, 13F flow, filings, options skew)
  - Short interest trend:
    - Shares short 3,314,735 vs 3,661,554 prior month (-9.5% — FALLING)
    - Days-to-cover 2.1
    - Short interest 1.3% of float
  - Institutional flow (13F deltas):
    - Top holders: 5 adding / 1 trimming this quarter
    - Blackrock Inc.: 8.10% held, +1.0% q/q
    - Vanguard Capital Management LLC: 5.87% held, +100.0% q/q
    - JPMORGAN CHASE & CO: 4.14% held, +9.8% q/q
    - State Street Corporation: 3.99% held, +1.1% q/q
    - Morgan Stanley: 2.77% held, -1.2% q/q
    - Vanguard Portfolio Management LLC: 2.25% held, +100.0% q/q
  - SEC filing stream:
    - 2026-06-25 11-K — Annual report of employee stock purchase
    - 2026-05-29 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-05-11 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-05-07 10-Q — Periodic Financial Reports
    - 2026-04-28 8-K ⚠️ — Corporate Changes & Voting Matters
  - Options skew / flow:
    - Reference expiry 2026-09-18 (~45d): ATM IV 33.4%
    - 25Δ put IV 32.9% vs 25Δ call IV 33.4% — skew -0.5% (balanced)
    - Same-day flow at this expiry: 5 call vol vs 7 put vol (C/P 0.71)

## Gapped data sources (penalize Confidence Score accordingly)
  - Congressional trades: Data Status: GAPPED
  - Earnings call transcripts: Data Status: GAPPED
  - Credit ratings: Data Status: GAPPED
