# COMMITTEE DATA PAYLOAD — CARR (Standard)
Generated: 2026-07-20 09:32 local | Source: stock-monitor local daemon
Instructions: Paste this payload into the Investment Committee session (Phase 1-4
protocol already loaded). Run the 9-member committee, score all 11 categories with
justifications, and output the matching Template.

## Company profile
- Sector: Industrials | Industry: Building Products & Equipment
- Next earnings: in 8 day(s)

## Trigger(s) breached
- Timing score Δ -23 (threshold ±15)

## Prior ledger state
- Previous Overall Score: 57/110
- Previous Timing Score: 48/100
- Previous Confidence: 70%
- Previous evaluation date: 2026-07-20 06:32

## Local proxy scores (deterministic noise gate — NOT the committee verdict)
- Overall (proxy): 54/110 → local rating band: Hold
- Timing: 25/100
- Data-completeness confidence (proxy): 70%
- Category proxies (1/11 are ESTIMATES from missing data — weigh accordingly):
  - Valuation: 8/10
  - Cash Generation: 1/10
  - Growth: 4/10
  - Revenue Visibility: 5/10  [ESTIMATE — source data missing, do not trust]
  - Margin Quality: 4/10
  - Balance Sheet: 7/10
  - Capital Allocation: 4/10
  - Macro Exposure: 8/10
  - Competitive Moat: 3/10
  - Analyst Sentiment: 8/10
  - Technical Position: 2/10

## Earnings gate (actionable-BUY policy: earnings within 10 business days AND
## strict positivity pass — score >=70, >=3 signals, beat streak >=3/4. A Buy
## rating without this gate is thesis-only, NOT actionable; committee should
## confirm or veto the positivity call using transcript/guidance judgment)
  - Earnings in business days: 6 (buy window: ≤10)
  - Earnings Positivity Score: 55/100 (strict pass bar 70, beat streak None/0 last quarters)
    - Analyst EPS revision momentum: 50
    - Earnings surprise history: GAPPED
    - Pre-earnings relative strength vs SPY: 54
    - Recommendation upgrades vs downgrades (90d): 50
    - Short interest level & trend: 77
  - GATE VERDICT: NOT ACTIONABLE — positivity 55/100 below strict bar 70; no reported-quarter history to verify beat streak

## Supermajority consensus vote (last gate before a BUY alert)
  - Verdict: FAIL — 2/10 systems bullish (supermajority 65%, min 6 computable)
    - momentum_timing: ✘
    - factor_tier_high: ✘
    - piotroski: ✘
    - canslim: ✘
    - estimate_revisions: ✔
    - insider_signal: ✘
    - momentum_6m: ✔
    - breakout_52wk: ✘
    - positivity_margin: ✘
    - quality_gates: ✘
  - Market regime: SPY>50d/200d, VIX 17.5 (max 28.0), term 0.888 — OK

## Evidence-backed factor screen (mechanical; committee should weigh explicitly)
  - Magic Formula rank: #55 of 109 in universe (earnings yield 3.3%, return on capital 44.0%)
  - Piotroski F-Score: 4/9 (8-9 strong, 0-2 weak)
  - PEG (GARP): 1.65
  - Momentum: 6-mo +21.6%, 12-mo -9.7%
  - Quality gates (Buffett-style): ROE >= 15%: FAIL; Debt/Equity < 0.5: FAIL; Gross margin > 40%: FAIL; CFO > Net Income: PASS
  - Graham/Buffett intrinsic value: Graham Number $23.37, margin of safety -190.0%
  - CANSLIM (O'Neil): 3/7 criteria met — C: N; A: N; N: N; S: N; L: Y; I: Y; M: Y
  - Dividend growth quality (if applicable): yield 1.4%, payout 62%, quality score 54/100, 5yr div CAGR +4.2%
  - Fama-French 5-factor tilts: Size: Large, Value tilt: Growth (descriptive only, not scored — see interpretation guide), Profitability: +25.9%, Asset growth: -0.6%
  - Quality Minus Junk (AQR-style): 57/100
  - Insider transactions (90d, cluster-buy check): 0 buy txns / 1 sell txns, 0 distinct buyers
  - Analyst estimate revision momentum: +1.0% (current vs 90 days ago)
  - 52-week high breakout (O'Neil/Darvas): -16.4% from 52-wk high
  - Interpretation guide: Magic Formula = Greenblatt cheapness+quality rank within
    the monitored universe; F-Score 8-9 strong / 0-2 weak (Piotroski); PEG <= 1
    attractive per Lynch GARP (ignore for cyclicals); 6-12 mo momentum is the
    Carhart UMD factor; quality gates are Buffett-style quantitative tells.
    Graham Number/margin of safety is asset-and-earnings based (1934-era value
    investing) — expect it to look deeply "overvalued" for asset-light,
    high-multiple growth/tech names; that's the methodology working as
    designed, not a flaw, and should be weighted down for this watchlist's
    growth names accordingly. CANSLIM (O'Neil) is a 7-criterion growth+
    breakout+market-regime checklist, not a continuous score — read the
    per-criterion breakdown, not just the count. Dividend quality only
    applies to actual payers (GAPPED for non-payers, not penalized). Fama-
    French Size/Value tilts are descriptive only and deliberately excluded
    from the conviction blend (a small-cap/cheap-book-to-market tilt would
    bias against this watchlist's mega-cap growth names) — Profitability/
    Investment legs are folded into Quality Minus Junk (AQR-style) instead.
    Insider signal only means something as a cluster (3+ distinct buyers,
    zero sales in 90 days per Lakonishok & Lee) — a single insider trade is
    noise. Analyst revision momentum compares current-quarter/next-year EPS
    estimates to 90 days ago; the 52-week breakout check is O'Neil/Darvas-
    style and distinct from the 3-month-high check in the momentum score above.

## Raw data fields (yfinance)
  - forwardPE: 21.121801
  - trailingPE: 45.183334
  - enterpriseToEbitda: 21.841
  - enterpriseToRevenue: 3.137
  - pegRatio: 1.652
  - freeCashflow: 849750016
  - operatingCashflow: 2108999936
  - totalRevenue: 21870000128
  - revenueGrowth: 0.024
  - earningsGrowth: -0.407
  - grossMargins: 0.25180998
  - operatingMargins: 0.06572
  - profitMargins: 0.059899997
  - ebitdaMargins: 0.14362
  - debtToEquity: 91.102
  - currentRatio: 1.05
  - totalCash: 1371000064
  - totalDebt: 12572999680
  - returnOnEquity: 0.09907
  - returnOnAssets: 0.031489998
  - beta: 1.309
  - recommendationMean: 1.95833
  - targetMeanPrice: 76.9419
  - currentPrice: 67.775
  - heldPercentInstitutions: 0.89185995
  - numberOfAnalystOpinions: 21

## Recent news headlines
  - Here's What to Expect From Carrier Global's Next Earnings Report (2026-07-14)
  - SPXC Climbs 26% in a Year: Is the Stock Poised for More Growth? (2026-07-13)
  - Carrier Global (CARR) Completes $440M Sale of Riello Business (2026-07-11)
  - Why Carrier Global (CARR) Stock Is Trading Up Today (2026-07-10)
  - 3 Reasons to Avoid CARR and 1 Stock to Buy Instead (2026-07-10)

## Recent insider transactions
  - DRYDEN EDWARD C —  (2026-07-01)
  - VIESSMANN MAXIMILIAN — Sale at price 62.01 per share. (2026-05-20)
  - PANDYA GAURANG —  (2026-05-01)
  - GIERGES MICHAEL LOTFY —  (2026-04-01)
  - HEIM THOMAS —  (2026-02-09)

## Depth enrichment (short interest, 13F flow, filings, options skew)
  - Short interest trend:
    - Shares short 15,718,853 vs 16,907,459 prior month (-7.0% — FALLING)
    - Days-to-cover 2.5
    - Short interest 2.3% of float
  - Institutional flow (13F deltas):
    - Top holders: 4 adding / 2 trimming this quarter
    - Capital Research Global Investors: 14.01% held, +55.7% q/q
    - Blackrock Inc.: 6.97% held, +0.9% q/q
    - Vanguard Capital Management LLC: 6.18% held, +100.0% q/q
    - Vanguard Portfolio Management LLC: 4.40% held, +100.0% q/q
    - JPMORGAN CHASE & CO: 4.21% held, -7.3% q/q
    - State Street Corporation: 4.03% held, -0.6% q/q
  - SEC filing stream:
    - 2026-05-29 SD — Specialized Disclosure Report filed pursuant to Section 1502 of the Do
    - 2026-04-30 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-04-16 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-03-03 DEF 14A — Proxy Statements
    - 2026-02-06 S-3ASR — Offering Registrations
  - Options skew / flow:
    - Reference expiry 2026-08-28 (~45d): ATM IV 48.4%
    - 25Δ put IV 49.2% vs 25Δ call IV 46.3% — skew +2.9% (balanced)
    - Same-day flow: 9 calls, no put volume

## Gapped data sources (penalize Confidence Score accordingly)
  - Congressional trades: Data Status: GAPPED
  - Earnings call transcripts: Data Status: GAPPED
  - Credit ratings: Data Status: GAPPED
