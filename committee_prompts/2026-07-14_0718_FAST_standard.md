# COMMITTEE DATA PAYLOAD — FAST (Standard)
Generated: 2026-07-14 07:18 local | Source: stock-monitor local daemon
Instructions: Paste this payload into the Investment Committee session (Phase 1-4
protocol already loaded). Run the 9-member committee, score all 11 categories with
justifications, and output the matching Template.

## Company profile
- Sector: Industrials | Industry: Industrial Distribution
- Next earnings: in 0 day(s)

## Trigger(s) breached
- Timing score Δ +21 (threshold ±15)

## Prior ledger state
- Previous Overall Score: 72/110
- Previous Timing Score: 55/100
- Previous Confidence: 70%
- Previous evaluation date: 2026-07-13 13:07

## Local proxy scores (deterministic noise gate — NOT the committee verdict)
- Overall (proxy): 74/110 → local rating band: Watch
- Timing: 76/100
- Data-completeness confidence (proxy): 70%
- Category proxies (1/11 are ESTIMATES from missing data — weigh accordingly):
  - Valuation: 5/10
  - Cash Generation: 4/10
  - Growth: 6/10
  - Revenue Visibility: 5/10  [ESTIMATE — source data missing, do not trust]
  - Margin Quality: 7/10
  - Balance Sheet: 10/10
  - Capital Allocation: 9/10
  - Macro Exposure: 9/10
  - Competitive Moat: 5/10
  - Analyst Sentiment: 6/10
  - Technical Position: 8/10

## Earnings gate (actionable-BUY policy: earnings within 10 business days AND
## strict positivity pass — score >=70, >=3 signals, beat streak >=3/4. A Buy
## rating without this gate is thesis-only, NOT actionable; committee should
## confirm or veto the positivity call using transcript/guidance judgment)
  - Earnings in business days: 0 (buy window: ≤10)
  - Earnings Positivity Score: 47/100 (strict pass bar 70, beat streak 1/4 last quarters)
    - Analyst EPS revision momentum: 54
    - Earnings surprise history: 38
    - Pre-earnings relative strength vs SPY: 36
    - Recommendation upgrades vs downgrades (90d): 50
    - Short interest level & trend: 66
  - GATE VERDICT: NOT ACTIONABLE — positivity 47/100 below strict bar 70; beat streak 1/4 quarters (strict bar: ≥3/4)

## Supermajority consensus vote (last gate before a BUY alert)
  - Verdict: FAIL — 5/9 systems bullish (supermajority 65%, min 6 computable)
    - momentum_timing: ✔
    - factor_tier_high: ✘
    - piotroski: ✔
    - canslim: ✘
    - estimate_revisions: ✔
    - insider_signal: GAPPED
    - momentum_6m: ✔
    - breakout_52wk: ✘
    - positivity_margin: ✘
    - quality_gates: ✔
  - Market regime: SPY>50d/200d, VIX 16.5 (max 28.0), term 0.853 — OK

## Evidence-backed factor screen (mechanical; committee should weigh explicitly)
  - Magic Formula rank: #56 of 115 in universe (earnings yield 3.1%, return on capital 39.6%)
  - Piotroski F-Score: 7/9 (8-9 strong, 0-2 weak)
  - PEG (GARP): 3.39
  - Momentum: 6-mo +17.9%, 12-mo +10.2%
  - Quality gates (Buffett-style): ROE >= 15%: PASS; Debt/Equity < 0.5: PASS; Gross margin > 40%: PASS; CFO > Net Income: PASS
  - Graham/Buffett intrinsic value: Graham Number $9.44, margin of safety -415.2%
  - CANSLIM (O'Neil): 4/7 criteria met — C: N; A: N; N: Y; S: N; L: Y; I: Y; M: Y
  - Dividend growth quality (if applicable): yield 1.9%, payout 79%, quality score 75/100, 5yr div CAGR +11.4%
  - Fama-French 5-factor tilts: Size: Large, Value tilt: Growth (descriptive only, not scored — see interpretation guide), Profitability: +45.0%, Asset growth: +7.6%
  - Quality Minus Junk (AQR-style): 74/100
  - Insider transactions (90d, cluster-buy check): 0 buy txns / 0 sell txns, 0 distinct buyers
  - Analyst estimate revision momentum: +1.1% (current vs 90 days ago)
  - 52-week high breakout (O'Neil/Darvas): -3.9% from 52-wk high
  - Interpretation guide: Magic Formula = Greenblatt cheapness+quality rank within
    the monitored universe; F-Score 8-9 strong / 0-2 weak (Piotroski); PEG <= 1
    attractive per Lynch GARP (ignore for cyclicals); 6-12 mo momentum is the
    Carhart UMD factor; quality gates are Buffett-style quantitative tells.
    Graham Number/margin of safety is asset-and-earnings based (1934-era value
    investing) — expect it to look deeply "overvalued" for asset-light,
    high-multiple growth/tech names; that's the methodology working as
    designed, not a flaw, and should be weighted down for this watchlist's
    growth names accordingly. CANSLIM (O'Neil) is a 7-criterion growth+
    breakout+market-regime checklist, not a continuous score — read the
    per-criterion breakdown, not just the count. Dividend quality only
    applies to actual payers (GAPPED for non-payers, not penalized). Fama-
    French Size/Value tilts are descriptive only and deliberately excluded
    from the conviction blend (a small-cap/cheap-book-to-market tilt would
    bias against this watchlist's mega-cap growth names) — Profitability/
    Investment legs are folded into Quality Minus Junk (AQR-style) instead.
    Insider signal only means something as a cluster (3+ distinct buyers,
    zero sales in 90 days per Lakonishok & Lee) — a single insider trade is
    noise. Analyst revision momentum compares current-quarter/next-year EPS
    estimates to 90 days ago; the 52-week breakout check is O'Neil/Darvas-
    style and distinct from the 3-month-high check in the momentum score above.

## Raw data fields (yfinance)
  - forwardPE: 35.351334
  - trailingPE: 42.6886
  - enterpriseToEbitda: 28.675
  - enterpriseToRevenue: 6.414
  - pegRatio: 3.393
  - freeCashflow: 903862528
  - operatingCashflow: 1412099968
  - totalRevenue: 8442800128
  - revenueGrowth: 0.124
  - earningsGrowth: 0.138
  - grossMargins: 0.44893003
  - operatingMargins: 0.2033
  - profitMargins: 0.15392
  - ebitdaMargins: 0.22368
  - debtToEquity: 11.167
  - currentRatio: 4.392
  - totalCash: 308600000
  - totalDebt: 445500000
  - returnOnEquity: 0.33842
  - returnOnAssets: 0.21195999
  - beta: 0.713
  - recommendationMean: 2.72222
  - targetMeanPrice: 47.68462
  - currentPrice: 48.665
  - heldPercentInstitutions: 0.88053
  - numberOfAnalystOpinions: 13

## Recent news headlines
  - Fastenal Stock Drops After Earnings—Its Growth Isn’t Enough (2026-07-14)
  - Fastenal’s (NASDAQ:FAST) Q2 CY2026 Sales Top Estimates (2026-07-14)
  - Fastenal (FAST) Q2 Earnings Meet Estimates (2026-07-14)
  - Fastenal Delivers 15% Revenue Growth as Second-Quarter Sales Top Forecasts (FAST) (2026-07-14)
  - Earnings To Watch: Fastenal (FAST) Reports Q2 Results Tomorrow (2026-07-13)

## Recent insider transactions
  - ANCIUS MICHAEL J — Conversion of Exercise of derivative security at price 44.90 per share. (2026-04-24)
  - WISECUP REYNE KAY — Conversion of Exercise of derivative security at price 13.00 per share. (2026-03-05)
  - WISECUP REYNE KAY — Sale at price 47.34 per share. (2026-03-05)
  - SATTERLEE SCOTT ALAN — Conversion of Exercise of derivative security at price 19.00 per share. (2026-01-23)
  - SATTERLEE SCOTT ALAN — Sale at price 44.19 per share. (2026-01-23)

## Depth enrichment (short interest, 13F flow, filings, options skew)
  - Short interest trend:
    - Shares short 30,905,562 vs 28,459,231 prior month (+8.6% — RISING)
    - Days-to-cover 4.2
    - Short interest 3.1% of float
  - Institutional flow (13F deltas):
    - Top holders: 4 adding / 1 trimming this quarter
    - Blackrock Inc.: 9.12% held, +0.0% q/q
    - Vanguard Capital Management LLC: 6.49% held, +100.0% q/q
    - Vanguard Portfolio Management LLC: 5.61% held, +100.0% q/q
    - State Street Corporation: 4.84% held, +1.5% q/q
    - Charles Schwab Investment Management, Inc.: 3.43% held, -16.7% q/q
    - Geode Capital Management, LLC: 3.39% held, +3.2% q/q
  - SEC filing stream:
    - 2026-07-14 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-06-23 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-06-12 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-06-04 11-K — Annual report of employee stock purchase
    - 2026-05-19 SD — Specialized Disclosure Report filed pursuant to Section 1502 of the Do
  - Options skew / flow:
    - Reference expiry 2026-08-21 (~45d): ATM IV 21.6%
    - 25Δ put IV 37.3% vs 25Δ call IV 39.3% — skew -2.0% (upside being chased (call skew))
    - Same-day flow at this expiry: 605 call vol vs 33 put vol (C/P 18.33)

## Gapped data sources (penalize Confidence Score accordingly)
  - Congressional trades: Data Status: GAPPED
  - Earnings call transcripts: Data Status: GAPPED
  - Credit ratings: Data Status: GAPPED
