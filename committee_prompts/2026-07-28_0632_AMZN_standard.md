# COMMITTEE DATA PAYLOAD — AMZN (Standard)
Generated: 2026-07-28 06:32 local | Source: stock-monitor local daemon
Instructions: Paste this payload into the Investment Committee session (Phase 1-4
protocol already loaded). Run the 9-member committee, score all 11 categories with
justifications, and output the matching Template.

## Company profile
- Sector: Consumer Cyclical | Industry: Internet Retail
- Next earnings: in 2 day(s)

## Trigger(s) breached
- Earnings imminent: 2 day(s) out — pre-earnings committee review

## Prior ledger state
- Previous Overall Score: 64/110
- Previous Timing Score: 0/100
- Previous Confidence: 70%
- Previous evaluation date: 2026-07-27 12:32

## Local proxy scores (deterministic noise gate — NOT the committee verdict)
- Overall (proxy): 64/110 → local rating band: Watch
- Timing: 0/100
- Data-completeness confidence (proxy): 70%
- Category proxies (1/11 are ESTIMATES from missing data — weigh accordingly):
  - Valuation: 7/10
  - Cash Generation: 1/10
  - Growth: 7/10
  - Revenue Visibility: 5/10  [ESTIMATE — source data missing, do not trust]
  - Margin Quality: 5/10
  - Balance Sheet: 8/10
  - Capital Allocation: 7/10
  - Macro Exposure: 8/10
  - Competitive Moat: 6/10
  - Analyst Sentiment: 9/10
  - Technical Position: 1/10

## Earnings gate (actionable-BUY policy: earnings within 10 business days AND
## strict positivity pass — score >=70, >=3 signals, beat streak >=3/4. A Buy
## rating without this gate is thesis-only, NOT actionable; committee should
## confirm or veto the positivity call using transcript/guidance judgment)
  - Earnings in business days: 2 (buy window: ≤10)
  - Earnings Positivity Score: 73/100 (strict pass bar 70, beat streak 3/4 last quarters)
    - Analyst EPS revision momentum: 75
    - Earnings surprise history: 98
    - Pre-earnings relative strength vs SPY: 45
    - Recommendation upgrades vs downgrades (90d): 50
    - Short interest level & trend: 73
  - GATE VERDICT: ACTIONABLE BUY WINDOW

## Supermajority consensus vote (last gate before a BUY alert)
  - Verdict: FAIL — 3/10 systems bullish (supermajority 65%, min 6 computable)
    - momentum_timing: ✘
    - factor_tier_high: ✘
    - piotroski: ✔
    - canslim: ✘
    - estimate_revisions: ✔
    - insider_signal: ✘
    - momentum_6m: ✘
    - breakout_52wk: ✘
    - positivity_margin: ✘
    - quality_gates: ✔
  - HARD VETO: hostile market regime: SPY>!50d/200d, VIX 18.7 (max 28.0), term 0.926
  - Market regime: SPY>!50d/200d, VIX 18.7 (max 28.0), term 0.926 — HOSTILE

## Call-option candidate (independent options engine; committee should judge premium vs expected move, not just direction)
  - Judge confluence: 1/4 judges bullish (need >=100% of >=3) — vol_mispricing ✔, trend ✘, catalyst ✘, flow_proxy ✘
  - HARD VETO: 6-mo momentum -6% < 0% (falling knife)
  - HARD VETO: price below 200d SMA
  - HARD VETO: hostile market regime: SPY>!50d/200d, VIX 18.7 (max 28.0), term 0.926
  - No contract proposed: against: catalyst, flow_proxy, trend; 6-mo momentum -6% < 0% (falling knife); price below 200d SMA; hostile market regime: SPY>!50d/200d, VIX 18.7 (max 28.0), term 0.926

## Evidence-backed factor screen (mechanical; committee should weigh explicitly)
  - Magic Formula rank: #58 of 105 in universe (earnings yield 3.9%, return on capital 21.9%)
  - Piotroski F-Score: 6/9 (8-9 strong, 0-2 weak)
  - PEG (GARP): 1.25
  - Momentum: 6-mo -5.5%, 12-mo -0.6%
  - Quality gates (Buffett-style): ROE >= 15%: PASS; Debt/Equity < 0.5: FAIL; Gross margin > 40%: PASS; CFO > Net Income: PASS
  - Graham/Buffett intrinsic value: Graham Number $87.87, margin of safety -163.6%
  - CANSLIM (O'Neil): 3/7 criteria met — C: Y; A: Y; N: N; S: N; L: N; I: Y; M: N
  - Dividend growth quality (if applicable): not a dividend payer
  - Fama-French 5-factor tilts: Size: Mega, Value tilt: Growth (descriptive only, not scored — see interpretation guide), Profitability: +50.3%, Asset growth: +30.9%
  - Quality Minus Junk (AQR-style): 58/100
  - Insider transactions (90d, cluster-buy check): 0 buy txns / 12 sell txns, 0 distinct buyers
  - Analyst estimate revision momentum: +8.4% (current vs 90 days ago)
  - 52-week high breakout (O'Neil/Darvas): -16.9% from 52-wk high
  - Interpretation guide: Magic Formula = Greenblatt cheapness+quality rank within
    the monitored universe; F-Score 8-9 strong / 0-2 weak (Piotroski); PEG <= 1
    attractive per Lynch GARP (ignore for cyclicals); 6-12 mo momentum is the
    Carhart UMD factor; quality gates are Buffett-style quantitative tells.
    Graham Number/margin of safety is asset-and-earnings based (1934-era value
    investing) — expect it to look deeply "overvalued" for asset-light,
    high-multiple growth/tech names; that's the methodology working as
    designed, not a flaw, and should be weighted down for this watchlist's
    growth names accordingly. CANSLIM (O'Neil) is a 7-criterion growth+
    breakout+market-regime checklist, not a continuous score — read the
    per-criterion breakdown, not just the count. Dividend quality only
    applies to actual payers (GAPPED for non-payers, not penalized). Fama-
    French Size/Value tilts are descriptive only and deliberately excluded
    from the conviction blend (a small-cap/cheap-book-to-market tilt would
    bias against this watchlist's mega-cap growth names) — Profitability/
    Investment legs are folded into Quality Minus Junk (AQR-style) instead.
    Insider signal only means something as a cluster (3+ distinct buyers,
    zero sales in 90 days per Lakonishok & Lee) — a single insider trade is
    noise. Analyst revision momentum compares current-quarter/next-year EPS
    estimates to 90 days ago; the 52-week breakout check is O'Neil/Darvas-
    style and distinct from the 3-month-high check in the momentum score above.

## Raw data fields (yfinance)
  - forwardPE: 23.305792
  - trailingPE: 27.688622
  - enterpriseToEbitda: 16.563
  - enterpriseToRevenue: 3.476
  - pegRatio: 1.2456
  - freeCashflow: 9806624768
  - operatingCashflow: 148531003392
  - totalRevenue: 742775980032
  - revenueGrowth: 0.166
  - earningsGrowth: 0.748
  - grossMargins: 0.50604
  - operatingMargins: 0.1314
  - profitMargins: 0.12224
  - ebitdaMargins: 0.20984
  - debtToEquity: 53.3
  - currentRatio: 1.177
  - totalCash: 143088992256
  - totalDebt: 235540004864
  - returnOnEquity: 0.24285
  - returnOnAssets: 0.06845
  - beta: 1.461
  - recommendationMean: 1.33333
  - targetMeanPrice: 313.0192
  - currentPrice: 231.2
  - heldPercentInstitutions: 0.68764
  - numberOfAnalystOpinions: 62

## Recent news headlines
  - Tech stocks today: Big Tech earnings this week mark a pivotal moment for the AI trade (2026-07-27)
  - Meta, Amazon: Why this analyst feels 'less confident' in one (2026-07-28)
  - Ryanair Extends AWS AI Partnership (2026-07-28)
  - Marvell Technology vs. Snowflake: Which High-Growth Artificial Intelligence Stock Is a Better Buy in 2026? (2026-07-28)
  - Can Amazon.com (AMZN) Stay Undervalued As Earnings Near? (2026-07-28)

## Recent insider transactions
  - HERRINGTON DOUGLAS J — Sale at price 239.77 per share. (2026-07-01)
  - HERRINGTON DOUGLAS J — Sale at price 266.19 per share. (2026-06-01)
  - ZAPOLSKY DAVID A. — Sale at price 261.96 - 268.53 per share. (2026-05-22)
  - GARMAN MATTHEW S — Sale at price 261.93 - 265.64 per share. (2026-05-21)
  - HERRINGTON DOUGLAS J — Sale at price 261.88 - 263.85 per share. (2026-05-21)

## Depth enrichment (short interest, 13F flow, filings, options skew)
  - Short interest trend:
    - Shares short 106,566,350 vs 98,953,140 prior month (+7.7% — RISING)
    - Days-to-cover 1.8
    - Short interest 1.1% of float
  - Institutional flow (13F deltas):
    - Top holders: 6 adding / 0 trimming this quarter
    - Blackrock Inc.: 6.84% held, +0.2% q/q
    - Vanguard Capital Management LLC: 5.87% held, +100.0% q/q
    - State Street Corporation: 3.63% held, +0.5% q/q
    - FMR, LLC: 3.33% held, +8.2% q/q
    - Geode Capital Management, LLC: 2.17% held, +3.6% q/q
    - Morgan Stanley: 1.62% held, +7.6% q/q
  - SEC filing stream:
    - 2026-07-09 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-06-12 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-05-29 SD — Specialized Disclosure Report filed pursuant to Section 1502 of the Do
    - 2026-05-22 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-05-04 DEFA14A — Proxy Statements
  - Options skew / flow:
    - Reference expiry 2026-09-04 (~45d): ATM IV 0.0%
    - Same-day flow at this expiry: 737 call vol vs 621 put vol (C/P 1.19)

## Gapped data sources (penalize Confidence Score accordingly)
  - Congressional trades: Data Status: GAPPED
  - Earnings call transcripts: Data Status: GAPPED
  - Credit ratings: Data Status: GAPPED
