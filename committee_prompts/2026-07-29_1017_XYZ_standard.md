# COMMITTEE DATA PAYLOAD — XYZ (Standard)
Generated: 2026-07-29 10:17 local | Source: stock-monitor local daemon
Instructions: Paste this payload into the Investment Committee session (Phase 1-4
protocol already loaded). Run the 9-member committee, score all 11 categories with
justifications, and output the matching Template.

## Company profile
- Sector: Technology | Industry: Software - Infrastructure
- Next earnings: in 7 day(s)

## Trigger(s) breached
- Timing score Δ +40 (threshold ±15)

## Prior ledger state
- Previous Overall Score: 53/110
- Previous Timing Score: 47/100
- Previous Confidence: 67%
- Previous evaluation date: 2026-07-24 07:19

## Local proxy scores (deterministic noise gate — NOT the committee verdict)
- Overall (proxy): 57/110 → local rating band: Hold
- Timing: 87/100
- Data-completeness confidence (proxy): 67%
- Category proxies (1/11 are ESTIMATES from missing data — weigh accordingly):
  - Valuation: 9/10
  - Cash Generation: 1/10
  - Growth: 4/10
  - Revenue Visibility: 5/10  [ESTIMATE — source data missing, do not trust]
  - Margin Quality: 1/10
  - Balance Sheet: 9/10
  - Capital Allocation: 3/10
  - Macro Exposure: 2/10
  - Competitive Moat: 5/10
  - Analyst Sentiment: 9/10
  - Technical Position: 9/10

## Earnings gate (actionable-BUY policy: earnings within 10 business days AND
## strict positivity pass — score >=70, >=3 signals, beat streak >=3/4. A Buy
## rating without this gate is thesis-only, NOT actionable; committee should
## confirm or veto the positivity call using transcript/guidance judgment)
  - Earnings in business days: 5 (buy window: ≤10)
  - Earnings Positivity Score: 86/100 (strict pass bar 70, beat streak None/0 last quarters)
    - Analyst EPS revision momentum: 78
    - Earnings surprise history: GAPPED
    - Pre-earnings relative strength vs SPY: 100
    - Recommendation upgrades vs downgrades (90d): 90
    - Short interest level & trend: 80
  - GATE VERDICT: NOT ACTIONABLE — no reported-quarter history to verify beat streak

## Supermajority consensus vote (last gate before a BUY alert)
  - Verdict: FAIL — 5/8 systems bullish (supermajority 65%, min 6 computable)
    - momentum_timing: ✔
    - factor_tier_high: ✘
    - piotroski: ✔
    - canslim: ✘
    - estimate_revisions: GAPPED
    - insider_signal: GAPPED
    - momentum_6m: ✔
    - breakout_52wk: ✘
    - positivity_margin: ✔
    - quality_gates: ✔
  - HARD VETO: hostile market regime: SPY>!50d/200d, VIX 19.7 (max 28.0), term 0.953
  - Market regime: SPY>!50d/200d, VIX 19.7 (max 28.0), term 0.953 — HOSTILE

## Evidence-backed factor screen (mechanical; committee should weigh explicitly)
  - Magic Formula rank: #32 of 116 in universe (earnings yield 6.1%, return on capital 23.4%)
  - Piotroski F-Score: 6/9 (8-9 strong, 0-2 weak)
  - PEG (GARP): 0.86 (GARP: attractive <= 1)
  - Momentum: 6-mo +27.6%, 12-mo +5.4%
  - Quality gates (Buffett-style): ROE >= 15%: FAIL; Debt/Equity < 0.5: PASS; Gross margin > 40%: PASS; CFO > Net Income: PASS
  - Graham/Buffett intrinsic value: Graham Number $32.59, margin of safety -152.8%
  - CANSLIM (O'Neil): 3/7 criteria met (2 gapped) — C: ?; A: ?; N: Y; S: N; L: Y; I: Y; M: N
  - Dividend growth quality (if applicable): not a dividend payer
  - Fama-French 5-factor tilts: Size: Large, Value tilt: Value (descriptive only, not scored — see interpretation guide), Profitability: +42.8%, Asset growth: +7.5%
  - Quality Minus Junk (AQR-style): 42/100
  - Insider transactions (90d, cluster-buy check): 0 buy txns / 0 sell txns, 0 distinct buyers
  - Analyst estimate revision momentum: GAPPED
  - 52-week high breakout (O'Neil/Darvas): -2.0% from 52-wk high
  - Interpretation guide: Magic Formula = Greenblatt cheapness+quality rank within
    the monitored universe; F-Score 8-9 strong / 0-2 weak (Piotroski); PEG <= 1
    attractive per Lynch GARP (ignore for cyclicals); 6-12 mo momentum is the
    Carhart UMD factor; quality gates are Buffett-style quantitative tells.
    Graham Number/margin of safety is asset-and-earnings based (1934-era value
    investing) — expect it to look deeply "overvalued" for asset-light,
    high-multiple growth/tech names; that's the methodology working as
    designed, not a flaw, and should be weighted down for this watchlist's
    growth names accordingly. CANSLIM (O'Neil) is a 7-criterion growth+
    breakout+market-regime checklist, not a continuous score — read the
    per-criterion breakdown, not just the count. Dividend quality only
    applies to actual payers (GAPPED for non-payers, not penalized). Fama-
    French Size/Value tilts are descriptive only and deliberately excluded
    from the conviction blend (a small-cap/cheap-book-to-market tilt would
    bias against this watchlist's mega-cap growth names) — Profitability/
    Investment legs are folded into Quality Minus Junk (AQR-style) instead.
    Insider signal only means something as a cluster (3+ distinct buyers,
    zero sales in 90 days per Lakonishok & Lee) — a single insider trade is
    noise. Analyst revision momentum compares current-quarter/next-year EPS
    estimates to 90 days ago; the 52-week breakout check is O'Neil/Darvas-
    style and distinct from the 3-month-high check in the momentum score above.

## Raw data fields (yfinance)
  - forwardPE: 16.250574
  - trailingPE: 64.375
  - enterpriseToEbitda: 31.881
  - enterpriseToRevenue: 2.052
  - pegRatio: 0.859
  - freeCashflow: -794561728
  - operatingCashflow: 3411973120
  - totalRevenue: 24478734336
  - revenueGrowth: 0.049
  - grossMargins: 0.45078
  - operatingMargins: -0.02632
  - profitMargins: 0.03297
  - ebitdaMargins: 0.06437
  - debtToEquity: 37.514
  - currentRatio: 1.986
  - totalCash: 7320968192
  - totalDebt: 8133390848
  - returnOnEquity: 0.03743
  - returnOnAssets: 0.02053
  - beta: 2.541
  - recommendationMean: 1.53333
  - targetMeanPrice: 93.225
  - currentPrice: 82.4
  - heldPercentInstitutions: 0.84571
  - numberOfAnalystOpinions: 40

## Recent news headlines
  - Block (XYZ) Earnings Expected to Grow: Should You Buy? (2026-07-29)
  - Visa Stock Rises as Report Signals 2,600 Job Cuts (2026-07-28)
  - Block vs. PayPal: Which Fintech Stock Is a Better Buy Right Now? (2026-07-28)
  - Judge Blocks Minnesota's Prediction Market Ban (2026-07-28)
  - PROG Holdings Q2 Preview: Leveraging Strengths Amid Market Woes (2026-07-28)

## Recent insider transactions
  - EISEN ANTHONY MATHEW — Sale at price 77.03 - 79.10 per share. (2026-07-24)
  - EISEN ANTHONY MATHEW — Sale at price 78.90 - 79.31 per share. (2026-07-21)
  - EISEN ANTHONY MATHEW — Sale at price 78.58 - 84.00 per share. (2026-07-16)
  - EISEN ANTHONY MATHEW — Sale at price 76.14 - 78.91 per share. (2026-07-13)
  - EISEN ANTHONY MATHEW — Sale at price 76.66 - 79.49 per share. (2026-07-08)

## Depth enrichment (short interest, 13F flow, filings, options skew)
  - Short interest trend:
    - Shares short 15,930,938 vs 20,303,763 prior month (-21.5% — FALLING)
    - Days-to-cover 2.6
    - Short interest 3.5% of float
  - Institutional flow (13F deltas):
    - Top holders: 6 adding / 0 trimming this quarter
    - Price (T.Rowe) Associates Inc: 8.45% held, +4.9% q/q
    - Blackrock Inc.: 7.89% held, +0.9% q/q
    - Vanguard Capital Management LLC: 6.65% held, +100.0% q/q
    - Vanguard Portfolio Management LLC: 4.80% held, +100.0% q/q
    - State Street Corporation: 4.32% held, +0.4% q/q
    - Capital World Investors: 3.85% held, +28.6% q/q
  - SEC filing stream:
    - 2026-06-18 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-06-05 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-05-26 SD — Specialized Disclosure Report filed pursuant to Section 1502 of the Do
    - 2026-05-07 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-05-04 8-K ⚠️ — Corporate Changes & Voting Matters
  - Options skew / flow:
    - Reference expiry 2026-09-18 (~45d): ATM IV 54.3%
    - 25Δ put IV 52.5% vs 25Δ call IV 51.7% — skew +0.9% (balanced)
    - Same-day flow at this expiry: 547 call vol vs 769 put vol (C/P 0.71)

## Gapped data sources (penalize Confidence Score accordingly)
  - Congressional trades: Data Status: GAPPED
  - Earnings call transcripts: Data Status: GAPPED
  - Credit ratings: Data Status: GAPPED
  - earningsGrowth: Data Status: GAPPED
