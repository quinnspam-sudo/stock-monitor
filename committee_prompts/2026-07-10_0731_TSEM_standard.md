# COMMITTEE DATA PAYLOAD — TSEM (Standard)
Generated: 2026-07-10 07:31 local | Source: stock-monitor local daemon
Instructions: Paste this payload into the Investment Committee session (Phase 1-4
protocol already loaded). Run the 9-member committee, score all 11 categories with
justifications, and output the matching Template.

## Company profile
- Sector: Technology | Industry: Semiconductors
- Next earnings: in 25 day(s)

## Trigger(s) breached
- Initial evaluation — no prior ledger entry

## Prior ledger state
- No prior evaluation on record (initial run).

## Local proxy scores (deterministic noise gate — NOT the committee verdict)
- Overall (proxy): 58/110 → local rating band: Hold
- Timing: 23/100
- Data-completeness confidence (proxy): 67%
- Category proxies (2/11 are ESTIMATES from missing data — weigh accordingly):
  - Valuation: 4/10
  - Cash Generation: 3/10
  - Growth: 7/10
  - Revenue Visibility: 5/10  [ESTIMATE — source data missing, do not trust]
  - Margin Quality: 6/10
  - Balance Sheet: 10/10
  - Capital Allocation: 4/10
  - Macro Exposure: 9/10
  - Competitive Moat: 3/10
  - Analyst Sentiment: 5/10  [ESTIMATE — source data missing, do not trust]
  - Technical Position: 2/10

## Earnings gate (actionable-BUY policy: earnings within 10 business days AND
## strict positivity pass — score >=70, >=3 signals, beat streak >=3/4. A Buy
## rating without this gate is thesis-only, NOT actionable; committee should
## confirm or veto the positivity call using transcript/guidance judgment)
  - Earnings in business days: 17 (buy window: ≤10)
  - Earnings Positivity Score: 91/100 (strict pass bar 70, beat streak 4/4 last quarters)
    - Analyst EPS revision momentum: 100
    - Earnings surprise history: 100
    - Pre-earnings relative strength vs SPY: 100
    - Recommendation upgrades vs downgrades (90d): 50
    - Short interest level & trend: 83
  - GATE VERDICT: NOT ACTIONABLE — earnings in 17 business days (window: ≤10)

## Supermajority consensus vote (last gate before a BUY alert)
  - Verdict: PASS — 6/9 systems bullish (supermajority 65%, min 6 computable)
    - momentum_timing: ✘
    - factor_tier_high: ✘
    - piotroski: ✔
    - canslim: ✔
    - estimate_revisions: ✔
    - insider_signal: GAPPED
    - momentum_6m: ✔
    - breakout_52wk: ✘
    - positivity_margin: ✔
    - quality_gates: ✔
  - Market regime: SPY>50d/200d, VIX 15.5 (max 28.0), term 0.823 — OK

## Evidence-backed factor screen (mechanical; committee should weigh explicitly)
  - Magic Formula rank: #101 of 121 in universe (earnings yield 1.0%, return on capital 8.1%)
  - Piotroski F-Score: 6/9 (8-9 strong, 0-2 weak)
  - PEG (GARP): 6.53
  - Momentum: 6-mo +87.5%, 12-mo +390.4%
  - Quality gates (Buffett-style): ROE >= 15%: FAIL; Debt/Equity < 0.5: PASS; Gross margin > 40%: FAIL; CFO > Net Income: PASS
  - Graham/Buffett intrinsic value: Graham Number $35.75, margin of safety -515.2%
  - CANSLIM (O'Neil): 5/7 criteria met — C: Y; A: Y; N: N; S: N; L: Y; I: Y; M: Y
  - Dividend growth quality (if applicable): not a dividend payer
  - Fama-French 5-factor tilts: Size: Large, Value tilt: Growth (descriptive only, not scored — see interpretation guide), Profitability: +23.2%, Asset growth: +7.8%
  - Quality Minus Junk (AQR-style): 53/100
  - Insider transactions (90d, cluster-buy check): 0 buy txns / 0 sell txns, 0 distinct buyers
  - Analyst estimate revision momentum: +16.3% (current vs 90 days ago)
  - 52-week high breakout (O'Neil/Darvas): -31.3% from 52-wk high
  - Interpretation guide: Magic Formula = Greenblatt cheapness+quality rank within
    the monitored universe; F-Score 8-9 strong / 0-2 weak (Piotroski); PEG <= 1
    attractive per Lynch GARP (ignore for cyclicals); 6-12 mo momentum is the
    Carhart UMD factor; quality gates are Buffett-style quantitative tells.
    Graham Number/margin of safety is asset-and-earnings based (1934-era value
    investing) — expect it to look deeply "overvalued" for asset-light,
    high-multiple growth/tech names; that's the methodology working as
    designed, not a flaw, and should be weighted down for this watchlist's
    growth names accordingly. CANSLIM (O'Neil) is a 7-criterion growth+
    breakout+market-regime checklist, not a continuous score — read the
    per-criterion breakdown, not just the count. Dividend quality only
    applies to actual payers (GAPPED for non-payers, not penalized). Fama-
    French Size/Value tilts are descriptive only and deliberately excluded
    from the conviction blend (a small-cap/cheap-book-to-market tilt would
    bias against this watchlist's mega-cap growth names) — Profitability/
    Investment legs are folded into Quality Minus Junk (AQR-style) instead.
    Insider signal only means something as a cluster (3+ distinct buyers,
    zero sales in 90 days per Lakonishok & Lee) — a single insider trade is
    noise. Analyst revision momentum compares current-quarter/next-year EPS
    estimates to 90 days ago; the 52-week breakout check is O'Neil/Darvas-
    style and distinct from the 3-month-high check in the momentum score above.

## Raw data fields (yfinance)
  - forwardPE: 38.2825
  - trailingPE: 102.16976
  - enterpriseToEbitda: 44.94
  - enterpriseToRevenue: 14.9
  - pegRatio: 6.5276
  - freeCashflow: 128945128
  - operatingCashflow: 811529024
  - totalRevenue: 1621565056
  - revenueGrowth: 0.155
  - earningsGrowth: 0.629
  - grossMargins: 0.24768
  - operatingMargins: 0.15609
  - profitMargins: 0.15131
  - ebitdaMargins: 0.33156
  - debtToEquity: 5.24
  - currentRatio: 5.63
  - totalCash: 1498547968
  - totalDebt: 155856000
  - returnOnEquity: 0.08691
  - returnOnAssets: 0.04146
  - beta: 0.873
  - targetMeanPrice: 313.83334
  - currentPrice: 219.665
  - heldPercentInstitutions: 0.76329005
  - numberOfAnalystOpinions: 6

## Recent news headlines
  - Marvell (MRVL) Lands 5 Million Tower PIC Shipments For AI Data Center Optics (2026-07-10)
  - Tower Semiconductor (TSEM) Advances AI Data Center Connectivity With Photonics Milestone (2026-07-09)
  - Tradr Introduces Leveraged ETFs on Ciena, Quantinuum, Rambus, Tower Semiconductor and TTM Technologies (2026-07-01)
  - Why Tower Semiconductor Took a Dive Today (2026-06-26)
  - Tradr to Launch Leveraged ETFs on CIEN, QNT, RMBS, TSEM & TTMI (2026-06-26)

## Recent insider transactions
  - GROSS DANA REBECCA — Stock Award(Grant) at price 0.00 per share. (2026-07-02)
  - CHELOUCHE YOAV — Stock Award(Grant) at price 0.00 per share. (2026-07-02)
  - ELSTEIN AMIR — Stock Award(Grant) at price 0.00 per share. (2026-07-02)
  - ELLWANGER RUSSELL CRAIG — Stock Award(Grant) at price 0.00 per share. (2026-07-02)
  - HASSON AVI — Stock Award(Grant) at price 0.00 per share. (2026-07-02)

## Depth enrichment (short interest, 13F flow, filings, options skew)
  - Short interest trend:
    - Shares short 2,637,271 vs 3,274,577 prior month (-19.5% — FALLING)
    - Days-to-cover 1.3
    - Short interest 2.4% of float
  - Institutional flow (13F deltas):
    - Top holders: 3 adding / 3 trimming this quarter
    - Price (T.Rowe) Associates Inc: 6.68% held, +57.1% q/q
    - Blackrock Inc.: 3.88% held, +9.3% q/q
    - Clal Insurance Enterprises Holdings Ltd: 3.61% held, -17.6% q/q
    - Phoenix Financial Ltd.: 3.08% held, -11.5% q/q
    - Vanguard Capital Management LLC: 2.80% held, +100.0% q/q
    - Senvest Management LLC: 2.06% held, -42.8% q/q
  - SEC filing stream:
    - 2026-07-07 6-K — Corporate Changes & Voting Matters
    - 2026-07-06 6-K — Corporate Changes & Voting Matters
    - 2026-06-22 6-K — Corporate Changes & Voting Matters
    - 2026-06-18 6-K — Corporate Changes & Voting Matters
    - 2026-06-15 6-K — Corporate Changes & Voting Matters
  - Options skew / flow:
    - Reference expiry 2026-08-21 (~45d): ATM IV 102.2%
    - 25Δ put IV 103.9% vs 25Δ call IV 100.7% — skew +3.2% (balanced)
    - Same-day flow at this expiry: 727 call vol vs 687 put vol (C/P 1.06)

## Gapped data sources (penalize Confidence Score accordingly)
  - Congressional trades: Data Status: GAPPED
  - Earnings call transcripts: Data Status: GAPPED
  - Credit ratings: Data Status: GAPPED
  - recommendationMean: Data Status: GAPPED
