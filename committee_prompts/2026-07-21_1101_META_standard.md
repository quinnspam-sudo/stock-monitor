# COMMITTEE DATA PAYLOAD — META (Standard)
Generated: 2026-07-21 11:01 local | Source: stock-monitor local daemon
Instructions: Paste this payload into the Investment Committee session (Phase 1-4
protocol already loaded). Run the 9-member committee, score all 11 categories with
justifications, and output the matching Template.

## Company profile
- Sector: Communication Services | Industry: Internet Content & Information
- Next earnings: in 8 day(s)

## Trigger(s) breached
- Earnings gate OPENED — earnings in 6 business day(s), positivity 70/100, beat streak 4/4 — buy decision is now actionable

## Prior ledger state
- Previous Overall Score: 89/110
- Previous Timing Score: 60/100
- Previous Confidence: 70%
- Previous evaluation date: 2026-07-21 10:46

## Local proxy scores (deterministic noise gate — NOT the committee verdict)
- Overall (proxy): 89/110 → local rating band: Strong Buy
- Timing: 60/100
- Data-completeness confidence (proxy): 70%
- Category proxies (1/11 are ESTIMATES from missing data — weigh accordingly):
  - Valuation: 8/10
  - Cash Generation: 4/10
  - Growth: 10/10
  - Revenue Visibility: 5/10  [ESTIMATE — source data missing, do not trust]
  - Margin Quality: 10/10
  - Balance Sheet: 9/10
  - Capital Allocation: 9/10
  - Macro Exposure: 9/10
  - Competitive Moat: 10/10
  - Analyst Sentiment: 9/10
  - Technical Position: 6/10

## Earnings gate (actionable-BUY policy: earnings within 10 business days AND
## strict positivity pass — score >=70, >=3 signals, beat streak >=3/4. A Buy
## rating without this gate is thesis-only, NOT actionable; committee should
## confirm or veto the positivity call using transcript/guidance judgment)
  - Earnings in business days: 6 (buy window: ≤10)
  - Earnings Positivity Score: 70/100 (strict pass bar 70, beat streak 4/4 last quarters)
    - Analyst EPS revision momentum: 67
    - Earnings surprise history: 100
    - Pre-earnings relative strength vs SPY: 32
    - Recommendation upgrades vs downgrades (90d): 50
    - Short interest level & trend: 72
  - GATE VERDICT: ACTIONABLE BUY WINDOW

## Supermajority consensus vote (last gate before a BUY alert)
  - Verdict: FAIL — 3/10 systems bullish (supermajority 65%, min 6 computable)
    - momentum_timing: ✘
    - factor_tier_high: ✘
    - piotroski: ✘
    - canslim: ✘
    - estimate_revisions: ✔
    - insider_signal: ✘
    - momentum_6m: ✔
    - breakout_52wk: ✘
    - positivity_margin: ✘
    - quality_gates: ✔
  - Market regime: SPY>50d/200d, VIX 17.0 (max 28.0), term 0.869 — OK

## Call-option candidate (independent options engine; committee should judge premium vs expected move, not just direction)
  - Judge confluence: 1/4 judges bullish (need >=100% of >=3) — vol_mispricing ✘, trend ✘, catalyst ✘, flow_proxy ✔
  - No contract proposed: against: catalyst, trend, vol_mispricing

## Evidence-backed factor screen (mechanical; committee should weigh explicitly)
  - Magic Formula rank: #33 of 106 in universe (earnings yield 5.3%, return on capital 33.0%)
  - Piotroski F-Score: 5/9 (8-9 strong, 0-2 weak)
  - PEG (GARP): 0.94 (GARP: attractive <= 1)
  - Momentum: 6-mo +7.4%, 12-mo -8.9%
  - Quality gates (Buffett-style): ROE >= 15%: PASS; Debt/Equity < 0.5: PASS; Gross margin > 40%: PASS; CFO > Net Income: PASS
  - Graham/Buffett intrinsic value: Graham Number $243.78, margin of safety -165.5%
  - CANSLIM (O'Neil): 4/7 criteria met — C: Y; A: Y; N: N; S: N; L: N; I: Y; M: Y
  - Dividend growth quality (if applicable): yield 0.3%, payout 8%, quality score 53/100, 5yr div CAGR -27.6%
  - Fama-French 5-factor tilts: Size: Mega, Value tilt: Growth (descriptive only, not scored — see interpretation guide), Profitability: +82.0%, Asset growth: +32.6%
  - Quality Minus Junk (AQR-style): 66/100
  - Insider transactions (90d, cluster-buy check): 0 buy txns / 15 sell txns, 0 distinct buyers
  - Analyst estimate revision momentum: +6.6% (current vs 90 days ago)
  - 52-week high breakout (O'Neil/Darvas): -18.7% from 52-wk high
  - Interpretation guide: Magic Formula = Greenblatt cheapness+quality rank within
    the monitored universe; F-Score 8-9 strong / 0-2 weak (Piotroski); PEG <= 1
    attractive per Lynch GARP (ignore for cyclicals); 6-12 mo momentum is the
    Carhart UMD factor; quality gates are Buffett-style quantitative tells.
    Graham Number/margin of safety is asset-and-earnings based (1934-era value
    investing) — expect it to look deeply "overvalued" for asset-light,
    high-multiple growth/tech names; that's the methodology working as
    designed, not a flaw, and should be weighted down for this watchlist's
    growth names accordingly. CANSLIM (O'Neil) is a 7-criterion growth+
    breakout+market-regime checklist, not a continuous score — read the
    per-criterion breakdown, not just the count. Dividend quality only
    applies to actual payers (GAPPED for non-payers, not penalized). Fama-
    French Size/Value tilts are descriptive only and deliberately excluded
    from the conviction blend (a small-cap/cheap-book-to-market tilt would
    bias against this watchlist's mega-cap growth names) — Profitability/
    Investment legs are folded into Quality Minus Junk (AQR-style) instead.
    Insider signal only means something as a cluster (3+ distinct buyers,
    zero sales in 90 days per Lakonishok & Lee) — a single insider trade is
    noise. Analyst revision momentum compares current-quarter/next-year EPS
    estimates to 90 days ago; the 52-week breakout check is O'Neil/Darvas-
    style and distinct from the 3-month-high check in the momentum score above.

## Raw data fields (yfinance)
  - forwardPE: 17.736952
  - trailingPE: 23.539076
  - enterpriseToEbitda: 15.049
  - enterpriseToRevenue: 7.653
  - pegRatio: 0.9424
  - freeCashflow: 25558249472
  - operatingCashflow: 124000002048
  - totalRevenue: 214962995200
  - revenueGrowth: 0.331
  - earningsGrowth: 0.624
  - grossMargins: 0.81941
  - operatingMargins: 0.40617
  - profitMargins: 0.32837
  - ebitdaMargins: 0.5085
  - debtToEquity: 35.608
  - currentRatio: 2.348
  - totalCash: 81180000256
  - totalDebt: 86769000448
  - returnOnEquity: 0.32930002
  - returnOnAssets: 0.16395001
  - beta: 1.246
  - recommendationMean: 1.34921
  - targetMeanPrice: 822.68585
  - currentPrice: 647.56
  - heldPercentInstitutions: 0.79907995
  - numberOfAnalystOpinions: 58

## Recent news headlines
  - Google Is Building an AI Chip Just for Gemini—And Investors Already Moved On It (2026-07-21)
  - Can AMD Narrow Hardware Gap With NVDA? This Analyst Sees Advancing AI 2026 Event As Key Catalyst (2026-07-21)
  - Meta’s Classic Wall Street Disconnect is the Opportunity I am Buying Over and Over (2026-07-21)
  - CoreWeave’s AI-Native Cloud Faces the Storm (2026-07-21)
  - Meta’s AI Push Threatens Shopify’s Core Business, Analyst Warns (2026-07-21)

## Recent insider transactions
  - OLIVAN JAVIER — Sale at price 660.98 - 661.13 per share. (2026-07-13)
  - OLIVAN JAVIER — Sale at price 600.00 - 603.14 per share. (2026-07-06)
  - KIMMITT ROBERT M — Sale at price 607.75 per share. (2026-07-01)
  - OLIVAN JAVIER — Sale at price 600.00 per share. (2026-06-15)
  - KIMMITT ROBERT M — Sale at price 629.29 per share. (2026-06-01)

## Depth enrichment (short interest, 13F flow, filings, options skew)
  - Short interest trend:
    - Shares short 34,489,980 vs 32,270,039 prior month (+6.9% — RISING)
    - Days-to-cover 1.8
    - Short interest 1.6% of float
  - Institutional flow (13F deltas):
    - Top holders: 3 adding / 3 trimming this quarter
    - Blackrock Inc.: 7.69% held, -1.6% q/q
    - Vanguard Capital Management LLC: 6.47% held, +100.0% q/q
    - FMR, LLC: 5.31% held, -4.7% q/q
    - State Street Corporation: 4.03% held, -2.5% q/q
    - Geode Capital Management, LLC: 2.47% held, +2.6% q/q
    - JPMORGAN CHASE & CO: 2.29% held, +14.4% q/q
  - SEC filing stream:
    - 2026-05-29 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-05-22 SD — Specialized Disclosure Report filed pursuant to Section 1502 of the Do
    - 2026-05-04 8-K ⚠️ — Corporate Changes & Voting Matters
    - 2026-04-30 10-Q — Periodic Financial Reports
    - 2026-04-29 8-K ⚠️ — Corporate Changes & Voting Matters
  - Options skew / flow:
    - Reference expiry 2026-08-28 (~45d): ATM IV 52.1%
    - 25Δ put IV 49.2% vs 25Δ call IV 52.0% — skew -2.8% (upside being chased (call skew))
    - Same-day flow at this expiry: 1,286 call vol vs 503 put vol (C/P 2.56)

## Gapped data sources (penalize Confidence Score accordingly)
  - Congressional trades: Data Status: GAPPED
  - Earnings call transcripts: Data Status: GAPPED
  - Credit ratings: Data Status: GAPPED
